...
import getUnixTimestamp from '../utils/date/getUnixTimestamp';
...
/* ── POST /api/evaluaciones/leccion/:idLeccion ──────────────
   Recibe los resultados de las evaluaciones por lección y se almacenan en evaluaciones_leccion

   Body: { respuestas: { total: number,correct: number } }  */
export async function enviarEvaluacion(req: Request, res: Response) {
  try {
    ...
    const {idLeccion, idSeccion} = req.params;

    const resultados = respuestas;

    //Calcular el porcentaje para almacenar la calificacion.
    console.log(resultados)

    const resultadosCorrectos = resultados.filter((res: any) => res.isCorrect === true);

    console.log(resultadosCorrectos);

    const porcentaje = Math.floor((resultadosCorrectos.length / resultados.length) * 100);

    console.log(porcentaje);

    const fechaRealizacion = getUnixTimestamp();

    const [secRows] = await pool.query<RowDataPacket[]>(
      `SELECT calificacion FROM evaluaciones_leccion WHERE idUsuario = ${idUsuario}`,
      [idLeccion]
    );

    console.log(secRows);
    //Guardar los resultados de la leccion

    if(secRows.length === 0){
      await conn.query(
         `INSERT INTO evaluaciones_leccion
            (idSeccion, idLeccion, idUsuario, calificacion, fechaRealizacion)
          VALUES (?, ?, ?, ?, ?)`,
         [parseInt(idSeccion), parseInt(idLeccion), idUsuario, porcentaje, fechaRealizacion]
       );
    }else{
      if(porcentaje > secRows[0].calificacion){
        await conn.query(
         `UPDATE evaluaciones_leccion SET calificacion = ${porcentaje} WHERE idUsuario = ${idUsuario} AND idLeccion = ${idLeccion}`,
       );
      }
    }
